...
也是使用复制算法的多线程收集器，它看上去和ParNew差不多，差别在侧重点不同。Paraller Scavenge关注点是吞吐量，所谓吞吐量就是cpu中用于运行用户代码的时间和总耗时占比。关注吞吐量适合多计算任务的场景，停顿时间适合多交互场景。
...
CMS是非常注重用户体验，以获取最短回收停顿时间为目标的收集器。

CMS是一种“标记-清除”算法实现的，它的过程分为以下4个步骤：
1.初始标记：stw，并记录下直接与root相连的对象，速度很快。 
2.并发标记：同时开启GC和用户线程，用一个闭包结构去记录可达对象，但在这个阶段，因为用户线程可能又有了新的引用。  
3.重新标记：stw，并发期间导致标记发生变动的那一部分对标记记录
。  
...
G1下的堆结构，它把内存分为像棋盘一样的块，每一块是region，大小的2的幂次，如果对象超过region大小怎么办，如果大于一个region，就申请两个region，并标记为H区。
Rset：每个region都存储一个rset，存储其他region引用当前的region对象。cset：这一次垃圾收集需要清理的垃圾集合。

YGC怎么做的呢？E区域和S0区域，把这两个区域存活的对象保存到STo区域，就是复制算法，只是说这里面E区和S区这些都是不连续的。
MixGC：因为G1里面没有单独做老年代的回收，老年代回收都会带上新生代，所以也叫MixGC，分为四个阶段：
1.初始标记 ：标记GCRoot直接引用的对象，只不过G1除了标记对象，也要标记对象所在的region，叫它Root Region，stw。
2.RootRegion ：也就是old区有谁引用的root region，如果有就标记这个region。
3.并发标记 ：只遍历root region引用的区域。
4.重新标价：satb。
5.清理：采用复制清理。stw。它的特点是选出来垃圾较多的old区域和所有的young区。
相比较于CMS，G1具备压缩功能避免碎片，让停顿时间更可控制，G1将java堆空间分隔成了若干大小相同的区域，既region，包括Eden、Servivor、Old、Humongous四个区域，Humongous是特殊的Old类型，专门放大对象。
